...
const PrisonersDilemmaGame = () => {
  ...
  const loadGameData = async () => {
    try {
      const statsData = await window.storage.get('player_stats');
      if (statsData) {
        setPlayerStats(JSON.parse(statsData.value));
      }
      ...
      const nameData = await window.storage.get('player_name');
      if (nameData) {
        setPlayerName(JSON.parse(nameData.value));
      }
    } catch (error) {
  ...
  const saveGameData = async (stats, history) => {
    try {
      await window.storage.set('player_stats', JSON.stringify(stats));
      await window.storage.set('game_history', JSON.stringify(history));
      if (playerName) {
        await window.storage.set('player_name', JSON.stringify(playerName));
      }
    } catch (error) {
  ...
  const resetGame = async () => {
    if (window.confirm('Are you sure you want to reset all progress?')) {
      try {
        await window.storage.delete('player_stats');
        await window.storage.delete('game_history');
        await window.storage.delete('player_name');
